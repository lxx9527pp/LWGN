...
class GraspRectangles:
    ...
    def load_from_array(cls, arr):
        """
        @classmethod
        Load grasp rectangles from numpy array.参数是列表,其中4表示四个点,2表示(x, y)是像素点的位置,N表示有N个抓取矩形框
        ...
        for i in range(arr.shape[0]):
            ...
            if grp.max() == 0:
                ...
                grs.append(GraspRectangle(grp))  # 输出的shape是(4, 2)即四个点表示一个抓取框所以装入列表是[(4, 2), (4, 2), ...]
        return cls(grs) 
        ...
        with open(fname) as f:
            while True:
                ...
                try:
                    ...
                    # 这里是一个GraspRectangle(gr)实例化的对象列表
                    grs.append(GraspRectangle(gr))

                except ValueError:
                    # Some files contain weird values.
                    continue
            # 最后返回的是将grs的列表作为参数实例化的GraspRectangles类对象
        return cls(grs)
    ...
    def draw(self, shape, position=True, angle=True, width=True):
        ...
        for gr in self.grs:
            rr, cc = gr.compact_polygon_coords(shape)  # 这个方法在GraspRectangle中
            if position:
                # 得到抓取标签的中心并把相对应的位置标记为1，即最高可信度的抓取位置其余位置为0
                # 这个方法是吧文本标签转化为图像
                # rr（行索引）代表了矩形框内所有包含的像素的行坐标。
                # cc（列索引）代表了矩形框内所有包含的像素的列坐标。
                pos_out[rr, cc] = 1.0
            if angle:
...
class GraspRectangle:
    ...
    def center(self):
        ...
        axis=0指的是在进行平均值计算时，选择沿着数组的第一个维度进行操作。
        如下面这个数组，axis=0就代表着延维度为0的方向做平均化
        [
    [x0, y0],
    [x1, y1],
    [x2, y2],
    ...
    def polygon_coords(self, shape=None):
        """
        :param shape: Output Shape
        :return: Indices of pixels within the grasp rectangle polygon（多边形）.
        """
    ...
    def iou(self, gr, angle_threshold=np.pi/6):
        ...
        return intersection/union  # 交叉/并集
    ...
    def rotate(self, angle, center):
        ...
        # print('points.shape:', self.points.shape, 'c.shape:', c.shape, 'R.shape', R.shape, '\n')
...
def detect_grasps(q_img, ang_img, width_img=None, no_grasps=1):
    ...
    """
    threshold_abs = 0.51 # 0.3
    min_distance = 1
    local_max = peak_local_max(q_img, min_distance=min_distance, threshold_abs=threshold_abs)
    while local_max.shape[0] > 25:
        threshold_abs += 0.05
        local_max = peak_local_max(q_img, min_distance=min_distance, threshold_abs=threshold_abs)
    ...
    for grasp_point_array in local_max:
        grasp_point = tuple(grasp_point_array)  # 输入抓取点的列表获取ang
